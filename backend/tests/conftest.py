...
# Stub heavy ML/optional deps so test collection works even without full env
import sys, types
for _m in ("cv2", "PIL", "PIL.Image", "ultralytics", "torch", "torchvision",
           "matplotlib", "segment_anything", "mobile_sam"):
...
@pytest.fixture
def client():
    """FastAPI TestClient WITHOUT admin auth stub.

    用于公共端点 (无需鉴权). Skips if app imports fail.
    """
    try:
        from main import app
    except ImportError as e:
