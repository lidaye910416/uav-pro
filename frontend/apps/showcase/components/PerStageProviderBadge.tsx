 ...
 *
 * 三种 variant:
 *  - compact (默认): 单一 pill, hover tooltip 展示 vision/decision 详情.
 *  - inline: 单一 inline span, 用于 hero 标题旁.
 *  - expanded: 拆为两个 chip (vision + decision), 中间 ↳.
 *
 * 颜色按 protocol (ollama=amber / anthropic=purple / openai=blue / 其他=blue).
 * 任何状态下 status.stages 缺失都回退到 status.display_name, 兼容旧后端.
 */
...
function shortModel(m: string | undefined): string {
  if (!m) return ""
  // 去掉 :tag 后缀 (例如 gemma4:e2b → gemma4)
  return m
}
...
export default function PerStageProviderBadge({
  ...
  // ── Fallback when status not yet loaded ────────────────────────────────────
  if (!status) {
    return (
      <Link
    ...
    if (isUnified) {
      ...
      return (
        ...
        >
          ...
          {showModel && status.model ? ` · ${shortModel(status.model)}` : ""}
        </span>
      )
    }
    return (
      <span
        ...
        title={status.description ?? ""}
      >
        <span style={{ color: colorFor(visionProvider) }}>
          vision={visionProvider}
        </span>
        <span style={{ color: "var(--text-muted)" }}>·</span>
        <span style={{ color: colorFor(decisionProvider) }}>
          ...
          <span>
            ...
            {showModel && (
              <span style={{ color: "var(--text-muted)" }}>
                {" · "}
                {shortModel(isUnified ? status.model : visionModel)}
              </span>
            )}
          </span>
        </span>
        ...
        >
          ...
          <span>
            ...
            {showModel && (
              <span style={{ color: "var(--text-muted)" }}>
                {" · "}
                {shortModel(isUnified ? status.model : decisionModel)}
              </span>
            )}
          </span>
        </span>
  ...
  const label = isUnified
    ...
    : `V:${visionProvider}/${shortModel(visionModel)} · D:${decisionProvider}/${shortModel(decisionModel)}`
