...
export interface LLMStatus {
  ...
  display_name: string
  description: string
  /** "unified" 两阶段 provider 一致; "per-stage" 至少一阶段 override. */
  scope?: "unified" | "per-stage"
  /** 结构化 per-stage 配置. 后端 GET /llm/status 已填充. */
  stages?: {
    vision: LLMStageStatus
    decision: LLMStageStatus
  }
}

/** 公开精简 schema: GET /llm/stages 返回 (无 base_url / api_key). */
export interface LLMPublicStageInfo {
  provider: string
  model: string
  protocol: string
  multimodal: boolean
...
export interface LLMPublicStages {
  scope: "unified" | "per-stage"
  vision: LLMPublicStageInfo
  decision: LLMPublicStageInfo
}

// LLM provider (admin)
export type LLMStageStatusAdmin = {
  provider: string
  model: string
  override_active: boolean
}
...
export type LLMProviderStatus = {
  ...
  stages: { vision: LLMStageStatusAdmin; decision: LLMStageStatusAdmin }
}
...
/** 公开 /llm/stages (无需鉴权, 不暴露 base_url / api_key). */
export const llmStagesPublicApi = {
  async get(): Promise<LLMPublicStages | null> {
    try {
      const res = await fetch(`${API_BASE}/llm/stages`)
      if (!res.ok) return null
      return (await res.json()) as LLMPublicStages
    } catch {
      return null
    }
  },
...
// LLM provider (admin) — uses LLMProviderStatus declared above
...
// Per-stage (vision / decision) LLM configuration (admin auth)
...
export interface LLMStageApplied {
  provider: string
  base_url: string
  api_key_set: boolean
  model: string
...
export interface LLMAdminPerStageResponse {
  vision: LLMStageApplied
  decision: LLMStageApplied
}

export const llmPerStageAdminApi = {
  async get(token: string): Promise<LLMAdminPerStageResponse> {
    const res = await fetch(`${API_BASE}/admin/llm/per-stage`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    })
    if (!res.ok) throw new Error(`per-stage ${res.status}`)
    return res.json() as Promise<LLMAdminPerStageResponse>
  },
  async update(
    token: string,
    body: { vision?: LLMStageConfig; decision?: LLMStageConfig },
  ): Promise<LLMAdminPerStageResponse> {
    const res = await fetch(`${API_BASE}/admin/llm/per-stage`, {
      method: "POST",
      headers: {
  ...
  async clear(token: string, stage: "vision" | "decision"): Promise<void> {
    const res = await fetch(`${API_BASE}/admin/llm/per-stage/${stage}`, {
      method: "DELETE",
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    })
    if (!res.ok) throw new Error(`per-stage-clear ${res.status}`)
  },
};

/** @deprecated use llmPerStageAdminApi for typed access; kept for back-compat. */
export const llmPerStageApi = llmPerStageAdminApi
